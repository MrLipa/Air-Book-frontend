...
const getUserIdByEmail = async (req, res) => {
  try {
    ...
    const userQuery = await pool.query(
      "SELECT user_id FROM air_book.users WHERE email = $1",
      [email]
    );
...
const getUserById = async (req, res) => {
  try {
    ...
    const userQuery = await pool.query(
      "SELECT * FROM air_book.users WHERE user_id = $1",
      [userId]
    );
...
const updateUserById = async (req, res) => {
  try {
    ...
    const missingFields = Object.entries(requiredFields).filter(([_, val]) =>
...
const patchUserById = async (req, res) => {
  try {
    ...
    const entries = Object.entries(updateFields)
      .filter(([key, value]) =>
        fieldMap[key] && value !== null && value !== undefined && value !== ""
      )
...
const deleteUserById = async (req, res) => {
  try {
    ...
    const result = await pool.query(
      "DELETE FROM air_book.users WHERE user_id = $1 RETURNING *",
      [userId]
    );
...
const getNotificationsByUserId = async (req, res) =>{
  try {
    ...
    const dbResult = await pool.query(
      `SELECT message FROM air_book.user_notifications WHERE user_id=$1`, [userId]
    )
